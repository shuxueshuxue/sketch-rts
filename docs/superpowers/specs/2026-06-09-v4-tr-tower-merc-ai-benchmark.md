...
- Remaining V4-TR stoplines are quality margin, not gate blockers: the latest audited low seed is `91/100`, and committed revision `0f00ef1` passes at `93/100`. The next useful work is not broad retreat/preservation changes but specific remaining timeout/loss clusters such as `thornedDelta`, `ochreRidge`, `lanternFord`, and `spruceCircuit`.
