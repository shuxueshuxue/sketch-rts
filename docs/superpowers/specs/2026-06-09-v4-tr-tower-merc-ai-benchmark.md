...
- Remaining V4-TR stoplines are quality margin, not gate blockers, but the current low race split against Ember V3 is exactly `45/50`. The next useful work is not broad retreat/preservation changes but specific Ember-loss and timeout clusters such as `thornedDelta`, `ochreRidge`, `lanternFord`, and `spruceCircuit`.
