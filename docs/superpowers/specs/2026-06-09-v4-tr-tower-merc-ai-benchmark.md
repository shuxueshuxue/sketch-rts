...
- Remaining V4-TR stoplines are quality margin, not gate blockers, but the current low seed is exactly `90/100` and the volatility is concentrated against Ember V3. The next useful work is not broad retreat/preservation changes but specific Ember-loss and timeout clusters such as `thornedDelta`, `ochreRidge`, `lanternFord`, and `spruceCircuit`.
