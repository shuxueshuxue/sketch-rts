...
- Mercenary-claim completion slice on 2026-06-08: exact `cedarPass v3 north` diagnosis showed squads parked idle at a reached mercenary camp while their `mercenary` claim stayed active because camps persist forever. Mercenary movement claims now complete after the grace window when a unit is idle on the camp point; the earlier next-decision claim-stability test still proves the claim is not dropped immediately.
- pgl evidence from the mercenary-claim completion slice: seed `v3-frozen-50-2026-06-08`, 50 maps / 100 matches, workers `95`, wall time `13.58s`, CPU time `606680.979ms`, V3 total `87/100`, Grove V3 `43/50`, Ember V3 `44/50`. The remaining same-seed losses are 7 Grove and 6 Ember.
- Current pgl dashboard evidence from branch `codex/v4-tr-benchmark` on 2026-06-09 shows the V3 gate now passes across three 50-map / 100-match seeds using the dedicated CLI and dashboard store:
  - `v3-frozen-50-2026-06-09-current`: run `2026-06-09T06-02-32-638Z-cewtgf`, workers `32`, wall time `13.52s`, CPU time `448609.019ms`, V3 total `90/100`, Grove V3 `43/50`, Ember V3 `47/50`.
  - `v3-frozen-50-2026-06-09b`: run `2026-06-09T06-03-16-221Z-11fekif`, workers `32`, wall time `14.97s`, CPU time `463452.969ms`, V3 total `93/100`.
  - `v3-frozen-50-2026-06-09c`: run `2026-06-09T06-03-17-886Z-115eytg`, workers `32`, wall time `15.49s`, CPU time `482341.339ms`, V3 total `91/100`.
  - `v3-frozen-50-2026-06-09d`: run `2026-06-09T06-13-10-688Z-10vfd4h`, workers `32`, wall time `16.57s`, CPU time `479508.581ms`, V3 total `91/100`.
  - `v3-frozen-50-2026-06-09e`: run `2026-06-09T06-13-11-774Z-10lfrfi`, workers `32`, wall time `15.29s`, CPU time `486187.156ms`, V3 total `92/100`.
- Enemy-side guarded mercenary objective slice on 2026-06-09: exact replay of `yarrowFen v3 south` showed V3 cleared its first natural, then `objectiveControl` redirected the opening army to the enemy-side guarded bow-post before the first expansion was built. V3 lost all workers and combat by 360s in the old replay. The planner now excludes enemy-side mercenary-camp guard clusters from ordinary objective control until the first expansion is established, while still allowing local guarded mercenary objectives and normal neutral objectives.
- Current pgl dashboard evidence after that slice, from branch `codex/v4-tr-benchmark` worktree head `7daae6f` plus local AI changes, uses workers `95` and the dedicated dashboard CLI:
  - `v3-frozen-current-audit-50-2026-06-09`: run `2026-06-09T20-40-05-443Z-s0wivd`, wall time `23.09s`, CPU time `596822.101ms`, V3 total `93/100`.
  - `v3-frozen-current-audit-50-2026-06-09b`: run `2026-06-09T20-40-06-207Z-1tejyep`, wall time `23.72s`, CPU time `606661.838ms`, V3 total `93/100`.
  - `v3-frozen-current-audit-50-2026-06-09c`: run `2026-06-09T20-40-06-465Z-1t4kcpq`, wall time `23.52s`, CPU time `623473.084ms`, V3 total `91/100`.
  - `v3-frozen-current-audit-50-2026-06-09d`: run `2026-06-09T20-45-19-037Z-1tyj5sn`, wall time `30.88s`, CPU time `616994.676ms`, V3 total `90/100`.
  - `v3-frozen-current-audit-50-2026-06-09e`: run `2026-06-09T20-45-17-445Z-1tojk3o`, wall time `29.27s`, CPU time `635292.046ms`, V3 total `90/100`.
- Rejected one-base neutral-route block on 2026-06-09: exact `frostMeadow v3 south` improved locally when V3/V2 one-base attack waves were blocked from stronger neutral routes, but full pgl dashboard runs dropped to `74/100` and `73/100` on seeds `v3-frozen-current-audit-50-2026-06-09d/e`. That route guard was reverted; do not reapply route blocking as a broad V2/V3 rule without preserving beneficial early pressure cases such as `russetBrook`.
- Enemy-base disadvantaged skirmish slice on 2026-06-09: exact `ashVale v3 north` showed V3 Grove expanding earlier but losing its small army after staying in a locally outmatched fight near a live enemy base. `skirmishPreservation` now allows V2/V3 local-skirmish retreat decisions inside enemy-base range instead of treating enemy-base proximity as a reason to skip preservation. Focused tests prove V3-style squads retreat from live-base disadvantage while V4-TR keeps tower/merc pressure committed.
- Current pgl dashboard evidence after the enemy-base skirmish slice, from branch `codex/v4-tr-benchmark` local worktree with workers `95`, shows the V3 gate has wider margin on the audited seeds:
...
- Current pgl dashboard evidence on 2026-06-10 from branch `codex/v4-tr-benchmark`, head `9f8774a` plus local AI worktree changes: seed `v3-current-audit-50-2026-06-10a`, run `2026-06-10T15-16-36-216Z-94le49`, workers `95`, wall time `10.13s`, CPU time `589289.597ms`, V3 total `94/100`.
...
- Veteran-core preservation slice on 2026-06-10: `skirmishPreservation` now treats `level >= 2` wounded combat units as accumulated combat capital and sends them through the existing healing-building recovery point before the ordinary critical-health cutoff. The RED test first returned `undefined` for a wounded level-3 knight; after the slice, the same scene returns a move command for that veteran. This reuses moon well / ember shrine recovery positioning and does not add a separate retreat path.
- pgl dashboard evidence after veteran-core preservation: seed `v3-veteran-core-50-2026-06-10a`, run `2026-06-10T19-10-56-612Z-50cj89`, workers `95`, wall time `9.77s`, CPU time `585961.472ms`, V3 total `92/100`.
...
- Dashboard CLI report slice on 2026-06-10: `benchmark:ai-v3-vs-prod-v2 -- --dashboard` now prints the same `byV3Race` and `byMatchup` breakdowns that the non-dashboard runner already computed, so the compact dashboard stdout no longer hides the Grove/Ember split behind only `primarySummary`.
- Report-layer verification for that slice: `npx vitest run scripts/ai-v3-vs-prod-v2-benchmark.test.ts scripts/ai-v4-tr-vs-v3-benchmark.test.ts src/ai/benchmark/control.test.ts src/ai/benchmark/dashboard-store.test.ts` passed with `25/25`, and `npm run build` completed successfully. This was a CLI output/reporting change only; the latest pgl AI-behavior gate remains the veteran-core `92/100` run above.
...
- Veteran-book feed slice on 2026-06-10: experience-book carrier scoring now gives extra weight to an existing veteran when the book can push that unit over the next star threshold. The RED test first gave the book to a fresh knight over a 2-star footman at 200xp; the kept scoring feeds the veteran instead.
