...
- Gauntlet CLI slice on 2026-06-08: `benchmark:ai-gauntlet` now exposes the 1v2 score, 1v3 probe, 2v3 probe, and controller-boundary robustness catalog as a first-class npm script. It accepts the shared `--seed`, `--map-count`, `--full`, `--dry-run`, `--runner-probe`, and `--match` flag surface; dry-run prints unique controller/lane match names and agent versions without executing matches, and `--match` filters by the same exact match name used by `play:ai --from-gauntlet`.
