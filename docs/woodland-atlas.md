...
- 30 种单位与中立生物：每种单独造型，按初级 / 进阶 / 精英区分装备档次。
- 13 种建筑：有明暗面的屋顶与墙体、旗帜、窗户、石基和功能标识。
...
单位造型按 catalog 的价格、人口与野怪营地食物分级（档次写在每个单位的卡片上，见 [`content/`](../src/client/content/)；由 `unit-art.test.ts` 对照 catalog 校验）：
...
精英档里，林野族是骑士和魔像，余烬是灰烬战殿训练的灰烬酋长（牛角盔、重斧、骨饰肩甲）与余烬复生者（缠着火焰、身上有发光裂纹）。四种精英都带重甲。

只有马厩训练的掠袭者（轻骑小马）与骑士（披甲战马）骑马；长枪兵、余烬劫掠者、余火奔袭者都是步兵。图鉴由 `unit-sheet.html` 直接调用游戏内绘制函数生成：本地运行 vite 后打开 `/unit-sheet.html` 截图即可。

## 实现位置

- [`content/`](../src/client/content/)：每个单位、每座建筑一张卡片，写中英文名称与说明、指令图标和快捷键、图形标识、美术档次和绘制函数。名称、提示框、指令面板、图形标识和图鉴页都从卡片推导。
- [`art/kit.ts`](../src/client/art/kit.ts)：所有绘制函数共用的画笔、配色、人形骨架、头饰、坐骑和建筑部件。
- [`atlas-art.ts`](../src/client/atlas-art.ts)：按卡片绘制单位与建筑，以及有上限的图形缓存。缓存分辨率随绘制尺寸调整，供战场、菜单插画和头像共用。
- [`atlas-theme.css`](../src/client/atlas-theme.css)：界面主题与响应式布局。
- [`main.ts`](../src/client/main.ts)：渲染接入、头像、指令图标与建造预览。
- [`game-shell.ts`](../src/client/game-shell.ts)、[`i18n.ts`](../src/client/i18n.ts)：菜单结构与界面文案（单位与建筑名称来自卡片）。
