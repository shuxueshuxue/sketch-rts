...
它的核心设计是：玩家输入、内置 AI、SDK agent、replay、benchmark、联机房间，最终都发出普通 command frame，进入同一个 simulation core。部署方式可以不同，但 gameplay core 不应该分叉。
...
- 联机走 command frame，不复制 gameplay 实现。
...
大型 AI benchmark 应该跑在远程 benchmark 机器上，不要烧本地笔记本。benchmark run 可以写入 dashboard store，并由 `benchmark.html` 展示。

## 一个 Gameplay Core

Sketch RTS 把 command 放在架构中心。
...
静态模式和 hosted 模式是产品/部署差异，不是两套 gameplay engine：

- 静态模式在浏览器里维护本地 room registry、本地 AI runtime、本地 command-frame adapter 和比赛生命周期。
- Hosted 模式用 HTTP 和 room lifecycle event stream 做房间设置，用 WebSocket room frame 做实时 lockstep gameplay。
- SDK 和 benchmark 复用同一套 command/simulation primitive，所以 AI 决策可以被 replay、测试、快进，或者直接放进房间里跑。

核心不变量：功能路径可以多，核心实现只能一套。
