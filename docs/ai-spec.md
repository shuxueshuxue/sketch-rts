...
- A stable score map must pass a same-policy side-swap control before it is allowed into the 64-map score pool. Run the same AI policy from both starting sides; if the same physical side wins both games, or both games repeatedly timeout without meaningful interaction, the map is measuring side bias instead of AI quality and is not a valid score map.
...
Anti-reward-hacking rule: benchmark lanes must be treated equally at the AI/runtime layer. `combat` may make failures faster to see, but it must not receive a bespoke think cadence, bespoke tactical brain, or lane-only conditions that are unavailable to `melee`. The exact think interval is not the point; it may be small, but it must be shared across all benchmark lanes: `1v2 score`, `1v1 score control`, `1v3 probe`, `2v3 probe`, `15v20 mixed combat`, and `10v12 mixed combat`. If we want faster thinking, lower the shared benchmark interval. Do not tune the interval, memory cadence, tactic cadence, or command arbitration per lane to rescue a metric. A difference is allowed only when it is a visible scenario rule with the same AI contract on both sides, not a hidden privilege for the lane. If a shorter think interval, tactical cadence, memory claim, command arbitration rule, focus-fire rule, item rule, or wounded-pullback rule is useful, it must be expressed as a shared SDK/AI capability that every lane can use under the same contract. A combat improvement only counts when it can migrate back into normal melee play; otherwise it is a benchmark exploit, not AI progress.
...
- The command surface is composable. High-level commands should cover common steering moves such as gather army, attack-move, focus fire, retreat, retreat wounded units by health threshold, mine, build, train, expand, creep a camp, hire mercenaries, use item, and step until a condition. Low-level raw commands remain available for exact control.
