...
describe("server deployment runtime", () => {
  ...
  it("routes room API and websocket URLs through the deployment base path", async () => {
    const calls: { path: string; body?: unknown }[] = [];
    const runtime = new ServerDeploymentRuntime({
    ...
    expect(calls.map((call) => call.path)).toEqual(["/sketch-rts/api/rooms?userId=user-1", "/sketch-rts/api/rooms"]);
