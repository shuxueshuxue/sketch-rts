...
function drawEffects(effects: WorldEffect[]) {
  for (const effect of effects) {
    ...
    if (effect.type === "chainLightning" && hasEffectVector(effect)) {
      ...
      drawChainLightningEffect(effect, from, to);
      continue;
    }
    ...
    if (effect.type === "heal") {
      drawHealEffect(effect, point, life);
      continue;
    }

    if (effect.type === "summon") {
      drawSummonEffect(point, life);
      continue;
    }

    if (effect.type === "curse") {
      drawCurseEffect(point, life);
      continue;
    }

    if (effect.type === "storm") {
      drawStormEffect(effect, point, life);
      continue;
    }

    if (effect.type === "guardianField") {
      drawGuardianFieldEffect(effect, point, life);
      continue;
    }
    ...
    if (effect.type === "flameBurn") {
      drawFlameBurnEffect(point, life);
      continue;
    }
...
function drawChainLightningEffect(effect: WorldEffect, from: Point, to: Point) {
  const life = effect.remaining / effect.duration;
...
function drawHealEffect(effect: WorldEffect, point: Point, life: number) {
  ctx.save();
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
...
function drawSummonEffect(point: Point, life: number) {
  const open = 1 - life;
  ctx.save();
  ctx.lineCap = "round";
...
function drawCurseEffect(point: Point, life: number) {
  const spread = 1 - life;
  ctx.save();
  ctx.lineCap = "round";
...
function drawStormEffect(effect: WorldEffect, point: Point, life: number) {
...
function drawGuardianFieldEffect(effect: WorldEffect, point: Point, life: number) {
...
function drawExperienceBurstEffect(point: Point, life: number) {
  const burst = 1 - life;
  ctx.save();
  ctx.lineCap = "round";
...
function drawFlameBurnEffect(point: Point, life: number) {
  const flare = 1 - life;
  ctx.save();
  ctx.lineCap = "round";
...
function strokePolyline(points: Point[]) {
  ctx.beginPath();
