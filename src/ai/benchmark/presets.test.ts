...
describe("AI benchmark presets", () => {
  ...
  it("builds the visible benchmark as melee score/probes plus tagged combat micro lanes", () => {
    ...
    expect(preset.input.evaluations[4]!.matches).toHaveLength(5);
    expect(preset.input.evaluations[5]!.matches).toHaveLength(5);
  ...
  it("uses matching combat recipes for both sides while varying mix, stars, and formation", () => {
    const preset = createAiVersionBenchmarkInput({ seed: "combat-recipes", mapCount: 18 });
    ...
    for (const evaluation of combatEvaluations) {
      expect(evaluation.matches.map((match) => match.name)).toEqual(
        expect.arrayContaining([
          expect.stringContaining("early mixed"),
          expect.stringContaining("ranged casters"),
          expect.stringContaining("high-star heavy"),
          expect.stringContaining("frontline healers"),
          expect.stringContaining("ranged spread"),
        ]),
      );
  ...
  it("runs combat lanes on a small arena where both AI versions actively fight", () => {
    const preset = createAiVersionBenchmarkInput({ seed: "combat-smoke", mapCount: 18, maxTicks: 1_500 });
  ...
  it("runs the combat micro targets and reports each recipe outcome separately", () => {
    const preset = createAiVersionBenchmarkInput({ seed: "combat-smoke", mapCount: 18, maxTicks: 9_000 });
    const combatEvaluations = preset.input.evaluations.filter((evaluation) => evaluation.tag === "combat");
    ...
    expect(summaries).toEqual([
      expect.objectContaining({ name: "15v20 mixed combat", matchCount: 5 }),
      expect.objectContaining({ name: "10v12 mixed combat", matchCount: 5 }),
    ]);
  ...
  it("starts combat lanes by sending v2 toward the enemy instead of back to its anchor", () => {
    const preset = createAiVersionBenchmarkInput({ seed: "combat-smoke", mapCount: 18, maxTicks: 45 });
    const combatEvaluation = preset.input.evaluations.find((evaluation) => evaluation.name === "15v20 mixed combat");
    if (!combatEvaluation) throw new Error("missing 15v20 combat evaluation");
  ...
  it("does not pass 1v1 score control when the opponent only died to neutral creeps", () => {
